...
`;
    ...
    if (data.yearsOfService) {
        prompt += `- 勤続年数: ${data.yearsOfService}年\n`;
    }
...
これから提示する「採用対象の部署」と「チームの補足情報」を最優先で考慮してください。
提供された既存のチームメンバーのプロファイルリストを分析し、指定された部署のバランス、多様性、および生産性を向上させるために、どのような人材を採用すべきかを提案してください。

分析のポイント:
- 指定された部署、または会社全体のチーム構成を分析する。
...
function formatExistingTeam(profiles: ComprehensiveDiagnosis[], department: string, teamContext: string): string {
    let formatted = `## 採用の背景
- 採用対象の部署: ${department}
...
const getTeamBuildingSuggestions_internal = async (payload: { profiles: ComprehensiveDiagnosis[], purpose: string, industry: string, teamSize: number, department: string }): Promise<TeamBuildingResult> => {
    const { profiles, purpose, industry, teamSize, department } = payload;
...
const getHiringRecommendation_internal = async (payload: { profiles: ComprehensiveDiagnosis[], department: string, teamContext: string }): Promise<HiringRecommendation> => {
    const { profiles, department, teamContext } = payload;
    const userPrompt = formatExistingTeam(profiles, department, teamContext);
