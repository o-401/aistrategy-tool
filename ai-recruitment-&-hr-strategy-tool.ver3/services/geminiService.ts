...
export const getTeamBuildingSuggestions = async (profiles: ComprehensiveDiagnosis[], purpose: string, industry: string, teamSize: number, department: string): Promise<TeamBuildingResult> => {
    if (profiles.length < teamSize) {
        throw new Error(`チーム分析には最低${teamSize}人のメンバーが必要ですが、現在${profiles.length}人しか選択されていません。`);
    }
    try {
        return await callApi<TeamBuildingResult>('getTeamBuildingSuggestions', { profiles, purpose, industry, teamSize, department });
    } catch (error) {
...
export const getHiringRecommendation = async (profiles: ComprehensiveDiagnosis[], department: string, teamContext: string): Promise<HiringRecommendation> => {
    if (profiles.length < 1) {
        throw new Error("チーム分析には最低1人のメンバーが必要です。");
    }
    try {
        return await callApi<HiringRecommendation>('getHiringRecommendation', { profiles, department, teamContext });
    } catch (error) {
