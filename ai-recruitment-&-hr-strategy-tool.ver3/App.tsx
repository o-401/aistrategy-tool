...
function App() {
  ...
  const handleTeamAnalysis = async (purpose: string, industry: string, teamSize: number, department: string) => {
      ...
      setError(null);
      try {
          const result = await getTeamBuildingSuggestions(profilesForAnalysis, purpose, industry, teamSize, department);
  ...
  const handleGetHiringRecommendation = async (department: string, teamContext: string) => {
    ...
    try {
        ...
        const result = await getHiringRecommendation(existingMemberDiagnoses, department, teamContext);
