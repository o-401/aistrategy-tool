...
export const ResultsScreen: React.FC<ResultsScreenProps> = ({ result, onAddToList, onRestart, appMode, onBack }) => {
      ...
      <div className="mt-8 pt-6 border-t border-slate-200">
        {onBack ? (
            <div className="text-center">
                 <button
                    onClick={onBack}
                    className="bg-slate-200 text-slate-700 font-bold py-3 px-8 rounded-full hover:bg-slate-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-slate-400 transition-transform duration-150 ease-in-out hover:scale-105"
                >
                    データ管理に戻る
                </button>
            </div>
        ...
        {(appMode === 'individual' || appMode === 'recruitment') && result.manager_view && (
            <div className="mt-8 flex justify-center">
                <div className="inline-flex rounded-lg shadow-sm">
                    ...
                    >
                        {onBack ? '本人用表示' : (appMode === 'recruitment' ? '候補者向け表示' : '本人用表示')}
                    </button>
                    <button
